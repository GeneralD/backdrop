...
| Async Bridge | `AsyncParsableCommand` | `AsyncRunnableCommandCommand` protocol — bridges `async run()` to sync `ParsableCommand` via `DispatchSemaphore`, keeping the main thread free for `NSApplication.run()` |
