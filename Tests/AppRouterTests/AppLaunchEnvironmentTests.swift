...
struct AppRouterTests {
    ...
    @Test("public init starts the default AppWindow and DisplayLinkDriver")
    func publicInitStartsDefaultWireframe() async {
        let router = AppRouter(
        ...
        await waitUntil {
            hasValue(named: "appWindow", from: router)
                && hasValue(named: "frameScheduler", from: router)
        }

        let appWindow: AnyObject? = value(named: "appWindow", from: router)
        let frameScheduler: AnyObject? = value(named: "frameScheduler", from: router)

        #expect(appWindow is AppWindow)
