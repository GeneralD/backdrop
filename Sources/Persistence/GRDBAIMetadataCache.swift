...
extension GRDBAIMetadataCache {
    public func read(rawTitle: String, rawArtist: String) async -> SearchCandidate? {
        try? await dbManager.dbQueue.read { db in
            ...
            return SearchCandidate(title: record.resolvedTitle, artist: record.resolvedArtist)
        }
    }

    public func write(rawTitle: String, rawArtist: String, candidate: SearchCandidate) async throws {
